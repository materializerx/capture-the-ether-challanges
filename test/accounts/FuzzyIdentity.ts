...
let accounts: Signer[];
let eoa: Signer;
let contractAsAttacker: Contract;
...
before(async () => {
  ...
  contractAsEOA = challengeFactory.attach(
    `0x197787d5200CD6486242597dD7FcECfFde525703`
...
it("solves the challenge", async function () {
  ...
  contractAsAttacker = await attackerFactory.connect(signer).deploy(contractAsEOA.address, {
    nonce: NONCE,
  });
  await signer.provider.waitForTransaction(contractAsAttacker.deployTransaction.hash);

  console.log(`attack ...`);
  tx = await contractAsAttacker.attack();
  await tx.wait();
